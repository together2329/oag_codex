...
| 7. Dispatch / Receipt | Give a fresh App Server worker thread bounded authority and verify its changed paths, execution manifest, and receipt. | `oag_dispatch.py create`, `oag_thread_worker.py`, allowed write paths, thread manifest, `oag_dispatch.py verify` | Required for post-lock RTL, TB, sim, lint, coverage, formal, SDC, signoff, or implementation filelist writes. |
