...
- `.codex/oag/feature-ipxact-policy.md` for product-visible feature scope,
  SSOT-inspired source sections, and IP-XACT-style integration projection.
  IP-XACT metadata may carry interfaces, registers, memory maps, parameters,
  file sets, hierarchy, and generator flow; OAG remains the authority for
  behavior, requirements, obligations, contracts, evidence, validation, and
  decisions.
...
- `oag-lock-preview-frame` and `.codex/scripts/oag_lock_preview_frame.py` for
  a formal pre-lock HTML review frame. The frame must render authored source
  artifacts verbatim with paths and hashes; it is review UI, not canonical
  truth.
...
- `.codex/oag/traceability-policy.md` for source-to-contract-to-evidence ID
  governance.
- `.codex/oag/ip-versioning-policy.md` for IP-local `.git`, functional
  semantic version bumps, golden baseline lineage, baseline manifest/tag
  ...
  Use `python3 .codex/scripts/oag_ip_git.py checkpoint --ip-dir <ip> --message "OAG <stage>: <summary>" --json`
  after meaningful OAG stage boundaries. The helper invokes `git` directly and
  is PowerShell-compatible when Git for Windows is installed; do not rely on
  `/bin/sh` or `sh.exe` for IP version checkpoints.
...
Before asking for lock on nontrivial work, produce a lock preview: draft
requirement atoms, candidate obligations, candidate assume/guarantee contracts,
decision-matrix rows, and verification intent. These are draft/proposed until
the user confirms lock; RTL/TB workers may only consume post-lock authoring
packets.
Render that preview with
`python3 .codex/scripts/oag_lock_preview_frame.py --ip-dir <ip> --json` so the
user can review `<ip>/knowledge/lock_preview/index.html`. The HTML must keep
the original source text in verbatim panels; any summary or issue table is only
navigation.
